...
load(file="0_Datos/Infobase/2020/Dori2020_v2.RData"   )
...
namevar <- c("IdDori", "IdDiario", "Nombre", "EsloraTotal", "CensoPorModalidad", "PuertoBase", "IdMarea", 
              "FcSalida","CodigoPuertoSalida_AL5", "C_FcRegreso",  "C_CodigoPuertoDesembarque_AL5", "C_FcVentaMax", "C_CodigoPuertoVenta_AL5",
...
funMareasTotales <- function(x) { 
  MareasTotales <- x %>% group_by( CensoPorModalidad, Nombre, EsloraTotal, IdMarea, C_FcRegreso , CatchCategory) %>% 
    summarise(PesoTotal = sum(PesoConsumoTotal+ PesoDescarte, na.rm=T)) 
  MareasTotales <- MareasTotales %>% dcast(CensoPorModalidad + Nombre + EsloraTotal + C_FcRegreso + IdMarea ~ CatchCategory,  value.var = "PesoTotal")
  MareasTotales <- MareasTotales %>% arrange(Nombre, C_FcRegreso, CensoPorModalidad, EsloraTotal, IdMarea)
  return(MareasTotales)
}

funCodigoOrigen <- function(x) { 
  CodigoOrigen<- x %>% group_by( CensoPorModalidad, Nombre, EsloraTotal, IdMarea, C_FcRegreso , CodigoOrigen) %>% summarise(PesoTotal = sum(PesoConsumoTotal+ PesoDescarte, na.rm=T)) %>%
    dcast(CensoPorModalidad + Nombre + EsloraTotal + C_FcRegreso + IdMarea ~ CodigoOrigen,  value.var = "PesoTotal")
  names(CodigoOrigen) [names(CodigoOrigen) == 0] <- "X0"
  return(CodigoOrigen)
}

funDesembarcado <- function(x) { 
  Desembarcado<- x %>% group_by( CensoPorModalidad, Nombre, EsloraTotal, IdMarea, C_FcRegreso , Desembarcado) %>% summarise(PesoTotal = sum(PesoConsumoTotal+ PesoDescarte, na.rm=T)) %>%
    dcast(CensoPorModalidad + Nombre + EsloraTotal + C_FcRegreso + IdMarea ~ Desembarcado,  value.var = "PesoTotal")
...
# # ####################################### #
# # Paso 1 - InfoCapturas0               ####
# # ####################################### #

# Todas las mareas con Capturalance0 tienen un IdMarea oficial (1256).
...
# mareas con captura lance 0 (1256)
    check <- subset(MareasTotales, !is.na(CapturasLance0) )
    head(check)
    dim(check)
# mareas con captura lance 0 y capturas ==0 en CapturasCalculadas (299)
    check <- subset(MareasTotales, !is.na(CapturasLance0) &!is.na(CapturasCalculadas) & CapturasCalculadas==0 )
    head(check)
    dim(check)
# mareas con captura lance 0 y capturas >0 en CapturasCalculadas (21)
    check <- subset(MareasTotales, !is.na(CapturasLance0) &!is.na(CapturasCalculadas) & CapturasCalculadas>0 )
    head(check)
    dim(check)
# mareas con captura lance 0 y capturas en CapturasCalculadas NA (933)
    ...
    table(check$CensoPorModalidad)
      ...
      sort(table(check$Nombre[check$CensoPorModalidad=="CERCO EN CANTABRICO NW"]), decreasing = T)
      sort(table(check$Nombre[check$CensoPorModalidad=="ARTES MENORES EN CANTABRICO NW"]), decreasing = T)
      
# Comparativa en N de mareas Total y N de mareas con captura lance 0 (3518)
check <- subset(MareasTotales, !is.na(CapturasLance0) & is.na(CapturasCalculadas) & is.na(Descartes))
...
# cerco
    subset(Dori[,namevar], IdMarea=="ESP-TRP-02674120200426221923") 
    subset(Dori[,namevar], IdMarea=="ESP-TRP-02674120200427050442") 
    subset(MareasTotales, Nombre=="ABRA DE AGUIÑO" & month(C_FcRegreso)%in% c(4,5)) 
    # 
    # @@ eliminar

    subset(Dori[,namevar], IdMarea=="ESP-TRP-02531520200501095542") 
    subset(Dori[,namevar], IdMarea=="ESP-TRP-02531520200503203456") 
    subset(MareasTotales, Nombre=="AGUSTIN DEUNA" & month(C_FcRegreso)%in% c(4,5)) 
    # 
    # @@ eliminar

    
    subset(Dori[,namevar], IdMarea=="ESP-TRP-02523020200310063424") 
    subset(MareasTotales, Nombre=="AITA RAMON" & month(C_FcRegreso)%in% c(2,3,4)) 
    # 
    # @@ eliminar
    
    subset(Dori[,namevar], IdMarea=="ESP-TRP-02615720200219145136") 
    subset(MareasTotales, Nombre=="GURE IMANOL" & month(C_FcRegreso)%in% c(2,3,4)) 
    # 
    # @@ eliminar
...
# artes menores
    ...
    subset(MareasTotales, Nombre=="AMETS" & month(C_FcRegreso)%in% c(8)) 
    ...
    subset(MareasTotales, Nombre=="ATXURRA ANAIAK" & month(C_FcRegreso)%in% c(4)) 
...
sort(table(check$Nombre), decreasing=T)
length(unique(check$IdMarea))

  subset(MareasTotales, Nombre=="ANTIGUOTARRAK" & month(C_FcRegreso)==7)
  subset(Dori[,namevar], IdMarea=="43101_2020-07-07") %>% summarise(PesoNotaVenta = sum(PesoNotaVenta * FactorConversion))
  subset(Dori[,namevar], IdMarea=="43101_2020-07-07")
  
  subset(MareasTotales, Nombre=="JON KURTZIO" & month(C_FcRegreso)%in% c(8,9))
  subset(Dori[,namevar], IdMarea=="12467136_2020-09-03")
  
  subset(MareasTotales, Nombre=="MADARI" )
  subset(Dori[,namevar], IdMarea=="ESP-99054457")
  subset(Dori[,namevar], IdMarea=="33708720_2020-09-01")
  
  
  subset(MareasTotales, Nombre=="SATANAS BI" )
  subset(MareasTotales, Nombre=="SIEMPRE CALAFATE" )
  subset(MareasTotales, Nombre=="PLAYA DEL ASTILLERU" )
  subset(MareasTotales, Nombre=="PYSBE BAT" )
  subset(MareasTotales, Nombre=="PADILLA ANAIAK" )
  subset(MareasTotales, Nombre=="BIHOTZ ALAI" )
  subset(MareasTotales, Nombre=="ATXURRA ANAIAK" )
  
  
  
  subset(MareasTotales, Nombre=="BETI OITZ" )
  subset(Dori[,namevar], IdMarea=="ESP-13660808")
  subset(Dori[,namevar], IdMarea=="16621767_2020-08-11")
...
# la mayoria tienen tambien lineas con origen 0 (NV) cuyo peso ha isdo igualado a cero. esas lineas son las que habría que quitar
# no elimino ninguna marea. 
check <- CodigoOrigen %>% filter ((!is.na(CA)) | (!is.na(DE) )  ) # mareas que  tienen pesos con origen CA/ DE
head(check)
tail(check)
check

subset(Dori[,namevar], IdMarea=="ESP-TRP-02306220200101231326")
...
length(unique(Dori$IdMarea))
    ...
    #revision general de loa datos

head(check2)
  check2 <- CodigoOrigen %>% filter ((is.na(CA)) & (is.na(DE) ) & !is.na(NV)) # mareas que solo tienen pesos con origen NV
  check2 <- CodigoOrigen %>% filter ((is.na(CA)) & (is.na(DE) ) & (is.na(NV) & X0>0)) # mareas que solo tienen pesos con origen 0 y tienen pesos. No hay
  check2 <- CodigoOrigen %>% filter ((is.na(CA)) & (is.na(DE) ) & (!is.na(X0) )) # mareas que solo tienen pesos con origen 0. peso=0. 58 mareas
  check2   # son mareas que tienen Codigo marea y tienen NV, pero de las que no se ha encontrado el logbook. su peso se iguala a cero..
  table(check2$Nombre)
  Dori %>% filter (IdMarea %in% check2$IdMarea) %>% group_by(Nombre) %>% summarise(PesoConsumo   = sum(PesoConsumo),
                                                                                   PesoNotaVenta = sum(PesoNotaVenta),
                                                                                   Ntrips        = length(unique(IdMarea))) %>% data.frame()
  

  subset(Dori[,namevar], IdMarea=="ESP-TRP-02711220201108160011")
...
i <- "ESP-TRP-00594820200207232314"
i <- "ESP-TRP-02659720200310042359"
i <- "ESP-TRP-02631120200522154848"
i <- "ESP-TRP-02306220200101231326"
...
subset(CodigoOrigen, Nombre=="ATXURRA ANAIAK" & month(C_FcRegreso)%in% c(2,3))
subset(MareasTotales, Nombre=="ATXURRA ANAIAK" & month(C_FcRegreso)%in% c(2,3))
...
MareasTotales %>% filter(CapturasCalculadas==0 & !is.na(Descartes))
  subset(Dori[,namevar], IdMarea=="ESP-TRP-02339420200818231809") 
  subset(MareasTotales, Nombre=="BERRIZ MATUTINA" & month(C_FcRegreso)%in% c(8,9,10))

  ##  cuando hay descarte, mantenemos la marea
...
head(check)
table(check$CensoPorModalidad)
subset(check, CensoPorModalidad=="ARTES MENORES EN CANTABRICO NW")
...
table(temp$CodigoOrigen)
    ...
    # ejemplos
    subset(Dori[,namevar], IdMarea=="ESP-TRP-02532520200225075047") 
    subset(Dori[,namevar], IdMarea=="ESP-TRP-02532520200303085918") 
    subset(MareasTotales, Nombre=="AITANA DEL MAR" & month(C_FcRegreso)%in% c(2,3))
    

    subset(Dori[,namevar], IdMarea=="ESP-TRP-02494620200930030831") 
    subset(Dori[,namevar], IdMarea=="ESP-TRP-02494620200930061410") 
    subset(Dori[,namevar], IdMarea=="ESP-TRP-02494620201006030155")
    subset(MareasTotales, Nombre=="BETI ZERUKO IZARRA" & month(C_FcRegreso)%in% c(9,10)) 
    ...
    subset(MareasTotales, Nombre=="MADRE CONSUELO" & month(C_FcRegreso)%in% c(4))
    ...
    subset(MareasTotales, Nombre=="KANTAURI" & month(C_FcRegreso)%in% c(4)) 
    ...
    subset(MareasTotales, Nombre=="BETI SAN LUIS" & month(C_FcRegreso)%in% c(8,9))
...
table(temp$CensoPorModalidad, useNA = "ifany")
...
unique(temp$IdMarea[temp$CensoPorModalidad=="RASCO EN CANTABRICO NW"])
      ...
      # artes menores
      subset(Dori[,namevar], IdMarea=="ESP-TRP-02498820200716082922") 
      subset(Dori[,namevar], IdMarea=="ESP-TRP-02498820200714140842") 
      subset(MareasTotales, Nombre=="ESTELA DEL CARMEN" & month(C_FcRegreso)%in% c(7))
      
      subset(Dori[,namevar], IdMarea=="ESP-TRP-02498820200720140037") 
      subset(Dori[,namevar], IdMarea=="ESP-TRP-02498820190819085534") 
      subset(Dori[,namevar], IdMarea=="ESP-TRP-02498820190823203604") 
      subset(MareasTotales, Nombre=="ESTELA DEL CARMEN" & month(C_FcRegreso)%in% c(8))
      ...
      # cerco
      subset(Dori[,namevar], IdMarea=="ESP-TRP-01521920200713000741")
      subset(MareasTotales, Nombre=="BETI SAN LUIS" & month(C_FcRegreso)%in% c(8,9))
      
      subset(Dori[,namevar], IdMarea=="ESP-TRP-02233220200615080238") 
      subset(MareasTotales, Nombre=="IRIGOYEN BERRIA" & month(C_FcRegreso)%in% c(6,7,8))
      
      # rasco
      subset(Dori[,namevar], IdMarea=="ESP-TRP-02301520200110141800") 
      subset(Dori[,namevar], IdMarea=="ESP-TRP-02301520190110072502") 
      subset(MareasTotales, Nombre=="CARABA" & month(C_FcRegreso)%in% c(1,2))
...
check <-  MareasTotales %>% filter(CapturasCalculadas==0 & is.na(Descartes))   
temp <- subset (Dori[,namevar], IdMarea %in% check$IdMarea & !(PesoCapturado>0 | PesoDesembarcado>0) & CensoPorModalidad=="CERCO EN CANTABRICO NW")
...
unique(substr(temp$IdMarea,1,3))  # todas las mareas tienen codigo oficial. las guardamos en una tabla a parte y luego las juntamos (Como InfoCapturaLance0)
 ...
 length(unique(temp$IdMarea))
 sum(unique(temp$PesoConsumo))
 sum(unique(temp$PesoConsumoBajoTalla))
 sum(unique(temp$PesoDescarte))
...
# # ####################################### #
   ...
   Dori_FIN <- rbind(Dori, Dori_captura0, Dori_lance0)
       ...
       sum(Dori_ini$PesoConsumoTotal, na.rm=TRUE)
       sum(Dori_FIN$PesoConsumoTotal, na.rm=TRUE)
       sum(Dori_ini$PesoDescarte, na.rm=TRUE)
       sum(Dori_FIN$PesoDescarte, na.rm=TRUE)   
       
       length(unique(Dori_ini$IdMarea))
       length(unique(Dori_FIN$IdMarea))
   
   
   save(Dori_FIN, file="0_Datos/Infobase/2020/Dori2020_v3.RData")
   
   

   
   aa<- subset(Dori_FIN, AZTI_BD!="Incorporar")
   aa<- aa%>% group_by(CensoPorModalidad) %>% summarise(NtripsEliminado = length(unique(IdMarea)),
                                                        PesoConsumo = sum(PesoConsumo),
                                                        PesoDesembarcado = sum(PesoDesembarcado),
                                                        PesoCapturado = sum(PesoCapturado),
                                                        PesoNotaVenta = sum( PesoNotaVenta))

   bb<- subset(Dori_FIN, AZTI_BD=="Incorporar")
   bb<- bb%>% group_by(CensoPorModalidad) %>% summarise(NtripsTotal= length(unique(IdMarea)),
                                                        PesoConsumo = sum(PesoConsumo),
                                                        PesoDesembarcado = sum(PesoDesembarcado),
                                                        PesoCapturado = sum(PesoCapturado),
                                                        PesoNotaVenta = sum( PesoNotaVenta))
   
   tableres <- aa %>% left_join(bb, by="CensoPorModalidad")
