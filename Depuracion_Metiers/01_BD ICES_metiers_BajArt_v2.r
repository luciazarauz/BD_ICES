...
#  Agrupar por tamaño
#
# Con este srcipt asignamos un metier a cada cabecera
######################################################### #
...
#Desembarcos
DB <-read.table("2020_ventas.csv", sep=",",dec=".",header=T, stringsAsFactors = FALSE)
  head(DB); dim(DB)
...
buques      <- read.csv("2020_buques.csv",header=T,sep=",",dec=".", stringsAsFactors = FALSE); head(buques)
...
buquesLLSGNS <- read.csv("2019_buques_LLS_GNS.csv",header=T,sep=";",dec=",", stringsAsFactors = FALSE); head(buquesLLSGNS)
...
DB$Trip_id_V_venta <- paste(DB$Nombre_Buque, as.character(DB$Fecha_venta), DB$Puerto_Venta, sep="_")
...
DB$Metier_Principal <- buquesLLSGNS$Metier.principal.2018[match(DB$Cod_UE, buquesLLSGNS$Codigo.UE)]
...
DB$Especie_ALFA3[DB$Especie_Oficial %in% c("Rapes Lophiidae")] <- "MNZ"
DB$Especie_ALFA3[DB$Especie_Oficial %in% c("Calamares Loligo spp.")] <- "SQZ"
DB$Especie_ALFA3[DB$Especie_Oficial %in% c("Rayas spp")] <- "RAJ"
...
# PTB                                       ####
ptb_met     <- summaryBy(Kg_Desembarcados ~ Nombre_Buque + Trip, data=ptb, FUN=sum, na.rm=TRUE)
